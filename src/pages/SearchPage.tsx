...
export const SearchPage: React.FC = () => {
  ...
  const searchResults = useMemo(() => {
    ...
    results.sort((a, b) => {
        ...
        if (!aTitleMatch && bTitleMatch) return 1;
        // Secondary sort by type if titles are not primary matches or both/neither are
        const typeOrder = ['pattern', 'learningPath', 'article'];
        const aTypeIndex = typeOrder.indexOf(a.type);
        const bTypeIndex = typeOrder.indexOf(b.type);
        if (aTypeIndex !== bTypeIndex) return aTypeIndex - bTypeIndex;
        return a.title.localeCompare(b.title); // Alphabetical if all else fails
    });
  ...
  const handleSearchSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    // Future: update URL search param here if desired: navigate(`?q=${searchTerm}`);
  };
      ...
      <form onSubmit={handleSearchSubmit} className="mb-8 max-w-xl mx-auto">
        <div className="relative">
          <input
            ...
            className={`w-full pl-10 pr-4 py-3 border border-border-medium rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-${AREUM_ACCENT_COLOR} focus:border-transparent bg-bg-primary text-text-primary`}
          />
      ...
      <div className="flex justify-center space-x-2 mb-8">
        {(['all', 'patterns', 'articles', 'learningPaths'] as const).map(type => (
            <Button
                ...
                size="sm"
            >
                {type === 'learningPaths' ? 'Learning Paths' : type.charAt(0).toUpperCase() + type.slice(1)}
            </Button>
        ))}
      </div>
      ...
      {searchResults.length > 0 && (
        <div className="space-y-6">
          {searchResults.map((item) => (
            <Card key={`${item.type}-${item.id}`} className="p-4 sm:p-6 hover:shadow-md transition-shadow duration-200">
              <Link to={
                  item.type === 'pattern' ? `/patterns/${item.slug}` :
                  item.type === 'article' ? `/notebook/${item.slug}` :
                  item.type === 'learningPath' ? `/learning-paths#${item.slug}` : '#' // For learning paths, an ID on the page might be useful
              }>
                <div className="flex flex-col sm:flex-row gap-4">
                    {item.type === 'pattern' && item.imageUrl && (
                         <img src={item.imageUrl} alt={item.title} className="w-full sm:w-32 h-32 object-cover rounded-md flex-shrink-0" />
                    )}
                  ...
                  <div className="flex-grow">
                    <span className={`text-xs font-semibold uppercase text-${AREUM_ACCENT_COLOR} dark:text-${AREUM_ACCENT_COLOR}`}>{item.type === 'learningPath' ? 'Learning Path' : item.type}</span>
