      ...
      {learningPathsData.length > 0 ? (
        <div className="space-y-10">
          {learningPathsData.map((path, index) => (
            <Card key={path.id} className="p-6 md:p-8 shadow-xl border border-border-light dark:border-border-light">
              <div className="flex flex-col md:flex-row items-start md:items-center gap-4 mb-4">
                <div className={`p-3 bg-${AREUM_ACCENT_COLOR}/10 dark:bg-${AREUM_ACCENT_COLOR}/20 rounded-full`}> {/* Adjusted opacity for dark */}
              ...
              <div className="prose dark:prose-invert max-w-none text-text-secondary space-y-6 
                ...
                <div className="p-4 bg-bg-accent dark:bg-bg-accent rounded-md">
                    ...
                    <div>
                        ...
                        <ul className="space-y-2 text-sm">
                            {path.helpfulReads.map((read, i) => (
                                <li key={i} className="p-3 bg-bg-primary dark:bg-bg-primary rounded shadow-sm border border-border-light dark:border-border-light">
