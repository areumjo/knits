...
export const PatternsPage: React.FC = () => {
  ...
  return (
    <div className="space-y-12">
      ...
      {spotlightPattern && (
        <section className="mb-12 p-6 bg-bg-accent rounded-lg shadow-lg">
      ...
      <div className="flex flex-col md:flex-row gap-8">
        <aside className={`md:w-1/4 ${showFilters ? 'block' : 'hidden'} md:block`}>
          <div className="sticky top-24 space-y-6 p-4 bg-bg-secondary rounded-lg shadow">
            <h3 className={`font-primary text-lg font-semibold text-text-primary flex items-center justify-between`}>
              Find Your Next Project
              <button onClick={() => setShowFilters(false)} className="md:hidden text-text-muted hover:text-text-primary">
                  ...
                  {category.options.map(option => (
                    <li key={option.value}>
                      <label className="flex items-center space-x-2 text-sm text-text-secondary cursor-pointer">
                        <input
                          type="checkbox"
                          className={`form-checkbox h-4 w-4 text-${AREUM_ACCENT_COLOR} rounded border-border-medium bg-bg-primary focus:ring-${AREUM_ACCENT_COLOR} checked:bg-${AREUM_ACCENT_COLOR} checked:border-${AREUM_ACCENT_COLOR}`}
        ...
        <main className="md:w-3/4">
          <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
            <button 
              ...
              className={`md:hidden flex items-center space-x-2 px-4 py-2 border border-border-medium rounded-md text-text-secondary hover:bg-bg-accent`}
            >
            ...
            <div className="flex items-center space-x-2">
              ...
              <select
                ...
                className={`block w-full sm:w-auto pl-3 pr-10 py-2 text-base bg-bg-primary border-border-medium text-text-primary focus:outline-none focus:ring-${AREUM_ACCENT_COLOR} focus:border-${AREUM_ACCENT_COLOR} rounded-md`}
              >
          ...
          {filteredAndSortedPatterns.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-6"> {/* Adjusted lg:grid-cols-2 for better fit in 3/4 width */}
       ...
       <div className="mt-12 text-center p-6 bg-bg-accent rounded-lg">
