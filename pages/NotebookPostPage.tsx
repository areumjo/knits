...
export const NotebookPostPage: React.FC = () => {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <article className="bg-bg-secondary dark:bg-bg-secondary p-6 sm:p-8 rounded-lg shadow-xl">
        <header className="mb-8 border-b border-border-light dark:border-border-light pb-6">
          ...
          <div className="flex flex-wrap items-center text-xs sm:text-sm text-text-muted space-x-4">
            ...
            <span className="flex items-center">
              <CalendarIcon className="h-4 w-4 mr-1.5" /> Published: {new Date(article.publishedDate).toLocaleDateString()}
            </span>
          </div>
          {article.tags && article.tags.length > 0 && (
            <div className="mt-4">
              {article.tags.map(tag => (
                <span key={tag} className="inline-block bg-bg-accent dark:bg-bg-accent rounded-full px-2.5 py-1 text-xs font-medium text-text-secondary mr-2 mb-1">
      ...
      {relatedArticles.length > 0 && (
        <aside className="mt-12 pt-8 border-t border-border-light dark:border-border-light">
          <h2 className="font-primary text-2xl font-semibold text-text-primary mb-6">
            Explore More On This Topic
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            {relatedArticles.map(related => (
              <Link key={related.id} to={`/notebook/${related.slug}`} className={`block p-4 bg-bg-accent dark:bg-bg-accent hover:bg-bg-soft dark:hover:bg-bg-soft rounded-lg shadow hover:shadow-md transition-shadow`}>
